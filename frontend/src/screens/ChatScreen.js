          ...
          <ScrollView style={{marginTop: 10}}>
            {history.map((item) => (
              <TouchableOpacity key={item.id} style={[styles.historyRow, activeChatId === item.id && styles.activeHistoryRow]} onPress={() => onSelectChat(item)}>
                <Ionicons name={item.type === 'chat' ? "chatbubble-outline" : "document-text-outline"} size={20} color={activeChatId === item.id ? "#007AFF" : "#666"} />
...
const AttachmentMenu = ({ isVisible, onUpload, onScan }) => {
  ...
  return (
    <View style={styles.plusMenuContainer}>
      ...
      <TouchableOpacity style={styles.plusMenuItem} onPress={onUpload}><Ionicons name="document" size={20} color="#333" /><Text style={styles.plusMenuText}>Upload Document</Text></TouchableOpacity>
      <TouchableOpacity style={styles.plusMenuItem} onPress={onScan}><Ionicons name="camera" size={20} color="#333" /><Text style={styles.plusMenuText}>Scan Document</Text></TouchableOpacity>
...
export default function ChatScreen({ route, navigation }) {
  ...
  const handleGoBack = () => { if (navigation.canGoBack()) navigation.goBack(); else navigation.navigate('HomeMain'); };
  const handleGoHome = () => { setIsMenuOpen(false); navigation.navigate('HomeMain'); };
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: '#fff' }, container: { flex: 1, backgroundColor: '#fff' },
  customHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 15, paddingVertical: 10, borderBottomWidth: 1, borderColor: '#F2F4F8', height: 60 },
  headerLeft: { flexDirection: 'row', alignItems: 'center' },
  headerRight: { width: 60, alignItems: 'flex-end' },
  iconButton: { padding: 5, marginRight: 5 },
  titleContainer: { flex: 1, alignItems: 'center' },
  headerTitle: { fontSize: 18, fontWeight: '700', color: '#1A1A1A' }, titleInput: { fontSize: 18, fontWeight: '700', textAlign: 'center', borderBottomWidth: 1, borderColor: '#007AFF', minWidth: 150 },
  ...
  sidebarHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 }, sidebarTitle: { fontSize: 22, fontWeight: 'bold' },
  
  // New Home Button Style
  homeBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F2F4F8', padding: 12, borderRadius: 8, marginBottom: 15 },
  homeBtnText: { color: '#333', fontWeight: '600', marginLeft: 10 },

  newChatBtn: { flexDirection: 'row', backgroundColor: '#007AFF', padding: 12, borderRadius: 8, justifyContent: 'center', marginBottom: 20 }, newChatText: { color: '#fff', fontWeight: '600', marginLeft: 8 },
  historyRow: { flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 8 }, activeHistoryRow: { backgroundColor: '#F0F8FF' }, historyLabel: { marginLeft: 10, fontSize: 16, color: '#333' }, activeHistoryLabel: { color: '#007AFF', fontWeight: '600' },
  listContent: { padding: 20, paddingBottom: 20 }, emptyState: { alignItems: 'center', marginTop: '40%' }, emptyText: { fontSize: 18, fontWeight: '700', color: '#ccc', marginTop: 15 },
  bubble: { maxWidth: '80%', padding: 14, borderRadius: 20, marginBottom: 12 }, userBubble: { alignSelf: 'flex-end', backgroundColor: '#007AFF', borderBottomRightRadius: 4 }, botBubble: { alignSelf: 'flex-start', backgroundColor: '#F2F4F8', borderBottomLeftRadius: 4 }, msgText: { fontSize: 16 }, userText: { color: '#fff' }, botText: { color: '#333' },
  inputContainer: { flexDirection: 'row', padding: 12, alignItems: 'center', borderTopWidth: 1, borderColor: '#F2F4F8', backgroundColor: '#fff' }, input: { flex: 1, backgroundColor: '#F8F9FA', borderRadius: 24, paddingHorizontal: 16, paddingVertical: 12, marginHorizontal: 10 }, attachBtn: { padding: 5 }, sendBtn: { backgroundColor: '#007AFF', width: 40, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center' },
  plusMenuContainer: { position: 'absolute', bottom: 80, left: 20, width: 200, backgroundColor: '#fff', borderRadius: 12, shadowColor: '#000', shadowOpacity: 0.15, shadowRadius: 8, elevation: 5, padding: 5 }, plusMenuHeader: { padding: 8, borderBottomWidth: 1, borderColor: '#eee' }, plusMenuWarning: { fontSize: 12, color: '#FF9500', fontWeight: '600' }, plusMenuItem: { flexDirection: 'row', alignItems: 'center', padding: 12 }, plusMenuText: { marginLeft: 10, fontSize: 15, color: '#333' }, plusMenuArrow: { position: 'absolute', bottom: -10, left: 20, width: 0, height: 0, borderLeftWidth: 10, borderRightWidth: 10, borderTopWidth: 10, borderStyle: 'solid', backgroundColor: 'transparent', borderLeftColor: 'transparent', borderRightColor: 'transparent', borderTopColor: '#fff' }
});